...
export default class Crawler {
  ...
  async start() {
    ...
    for (const result of keywordResults) {
      ...
      const mergeKeyword = _article.map((ar) => ({ ...ar, keyword: result.keyword }));
    ...
    const result = await Promise.all(
      articles.map(async (article) => {
        const checkExist = await firestore.getData("article", "title", article.title as string);

        if (checkExist.length > 0) return null;
        else return (await firestore.addData("article", article)).id;
      })
    );
    console.log(result);
      ...
      elements.forEach((element) => {
        switch (element.nodeName) {
          case "A":
            ...
            break;
          case "IMG":
            if (element.className === "l hs bx hn ho ec") setObjectKey(obj, "avatarImageUrl", (element as any)?.src);
